...
1. Explain the difference between `Relational Databases` and `SQL`.
   ...
   A relational database is the actual database where the data is stored. SQL is a query language which is use to interact with the database.

2. Why do tables need a `primary key`?
   The primary key is necessary because it makes sure that each row has a unique value that you can use to access that row.

3. What is the name given to a table column that references the primary key on another table.
   Foreign Key

4. What do we need in order to have a _many to many_ relationship between two tables.
   We need to create an Intermediary table, which is a table that has IDs for two other tables that you want to be related to eachother in a many to many relationship.
